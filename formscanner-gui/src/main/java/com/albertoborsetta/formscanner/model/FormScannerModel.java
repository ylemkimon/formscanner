...
public class FormScannerModel {
	...
	private final FormScannerDesktop view;
	...
	public FormScannerModel(FormScannerDesktop view)
		...
		String installationLanguage = StringUtils.defaultIfBlank(System.getProperty("FormScanner_LANGUAGE"), System.getenv("FormScanner_LANGUAGE"));
		...
		templatePath = configurations.getProperty(FormScannerConfigurationKeys.TEMPLATE_SAVE_PATH, templatePath + "/FormScanner/templates/");
		resultsPath = configurations.getProperty(FormScannerConfigurationKeys.RESULTS_SAVE_PATH, resultsPath + "/FormScanner/results/");
		...
		threshold = configurations.getProperty(FormScannerConfigurationKeys.THRESHOLD, FormScannerConfigurationKeys.DEFAULT_THRESHOLD);
		density = configurations.getProperty(FormScannerConfigurationKeys.DENSITY, FormScannerConfigurationKeys.DEFAULT_DENSITY);
		shapeSize = configurations.getProperty(FormScannerConfigurationKeys.SHAPE_SIZE, FormScannerConfigurationKeys.DEFAULT_SHAPE_SIZE);
		shapeType = ShapeType.valueOf(configurations.getProperty(FormScannerConfigurationKeys.SHAPE_TYPE, FormScannerConfigurationKeys.DEFAULT_SHAPE_TYPE));
		cornerType = CornerType.valueOf(configurations.getProperty(FormScannerConfigurationKeys.CORNER_TYPE, FormScannerConfigurationKeys.DEFAULT_CORNER_TYPE));

		fontType = configurations.getProperty(FormScannerConfigurationKeys.FONT_TYPE, FormScannerConfigurationKeys.DEFAULT_FONT_TYPE);
		fontSize = configurations.getProperty(FormScannerConfigurationKeys.FONT_SIZE, FormScannerConfigurationKeys.DEFAULT_FONT_SIZE);
	...
	public void setDefaultFramePositions() {
		for (Frame frame : Frame.values()) {
			...
			if (pos == null) {
				...
				position = new Rectangle(Integer.parseInt(positions[0]), Integer.parseInt(positions[1]), Integer.parseInt(positions[2]), Integer.parseInt(positions[3]));
			}
		...
		case RENAME_FILES_FIRST:
			if (!openedFiles.isEmpty()) {
				...
				try {
					...
					Logger.getLogger(FormScannerModel.class.getName()).log(Level.DEBUG, null, e);
				}
			}
			break;
		...
		if (formTemplate != null) {
			...
			case ANALYZE_FILES_ALL:
				if (!openedFiles.isEmpty()) {
					...
					File outputFile = new File(resultsPath + FormScannerTranslation.getTranslationFor(FormScannerTranslationKeys.RESULTS_DEFAULT_FILE) + "_" + sdf.format(today) + ".csv");
					File savedFile = fileUtils.saveCsvAs(outputFile, filledForms, true);
					configurations.setProperty(FormScannerConfigurationKeys.RESULTS_SAVE_PATH, FilenameUtils.getFullPath(savedFile.getAbsolutePath()));
			...
			JOptionPane.showMessageDialog(null, FormScannerTranslation.getTranslationFor(FormScannerTranslationKeys.TEMPLATE_NOT_FOUND), FormScannerTranslation.getTranslationFor(FormScannerTranslationKeys.TEMPLATE_NOT_FOUND_POPUP), JOptionPane.WARNING_MESSAGE);
		}
	}
	...
	public void saveTemplate(boolean notify) {
		...
		if (template != null) {
			if (notify) {
				JOptionPane.showMessageDialog(null, FormScannerTranslation.getTranslationFor(FormScannerTranslationKeys.TEMPLATE_SAVED), FormScannerTranslation.getTranslationFor(FormScannerTranslationKeys.TEMPLATE_SAVED_POPUP), JOptionPane.INFORMATION_MESSAGE);
			}
			...
			if (notify) {
				JOptionPane.showMessageDialog(null, FormScannerTranslation.getTranslationFor(FormScannerTranslationKeys.TEMPLATE_NOT_SAVED), FormScannerTranslation.getTranslationFor(FormScannerTranslationKeys.TEMPLATE_NOT_SAVED_POPUP), JOptionPane.ERROR_MESSAGE);
			}
		}
	}
	...
	private boolean openTemplate(File template, boolean notify) {
		...
		try {
			...
			threshold = formTemplate.getThreshold() < 0
					? threshold : formTemplate.getThreshold();
			density = formTemplate.getDensity() < 0
					? density : formTemplate.getDensity();
			shapeSize = formTemplate.getSize() < 0
					? shapeSize : formTemplate.getSize();
			shapeType = formTemplate.getShape() == null
			...
			if (notify) {
				JOptionPane.showMessageDialog(null, FormScannerTranslation.getTranslationFor(FormScannerTranslationKeys.TEMPLATE_LOADED), FormScannerTranslation.getTranslationFor(FormScannerTranslationKeys.TEMPLATE_LOADED_POPUP), JOptionPane.INFORMATION_MESSAGE);
			}
			templatePath = FilenameUtils.getFullPath(template.getAbsolutePath());
			String templateFile = FilenameUtils.getName(template.getAbsolutePath());
			configurations.setProperty(FormScannerConfigurationKeys.TEMPLATE, templateFile);
			...
			return true;
				...
				| HeadlessException e) {
			if (notify) {
				JOptionPane.showMessageDialog(null, FormScannerTranslation.getTranslationFor(FormScannerTranslationKeys.TEMPLATE_NOT_LOADED), FormScannerTranslation.getTranslationFor(FormScannerTranslationKeys.TEMPLATE_NOT_LOADED_POPUP), JOptionPane.ERROR_MESSAGE);
			}
			Logger.getLogger(FormScannerModel.class.getName()).log(Level.DEBUG, null, e);
			return false;
		}
	...
	public void setLanguage(String name) {
		...
		configurations.store();
		JOptionPane.showMessageDialog(null, FormScannerTranslation.getTranslationFor(FormScannerTranslationKeys.LANGUAGE_CHANGED), FormScannerTranslation.getTranslationFor(FormScannerTranslationKeys.SETTINGS_POPUP), JOptionPane.INFORMATION_MESSAGE);
	}
