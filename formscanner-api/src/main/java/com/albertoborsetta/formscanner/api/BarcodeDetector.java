	...
	public HashMap<String, FormArea> call() throws Exception {
	    ...
	    try {
	      ...
	      try {
	        ...
	        Result[] theResults = multiReader.decodeMultiple(bitmap, Constants.HINTS);
	        if (theResults != null) {
	      ...
	      if (results.isEmpty()) {
	        try {
	          ...
	          Result theResult = reader.decode(bitmap, Constants.HINTS_PURE);
	          if (theResult != null) {
	      ...
	      if (results.isEmpty()) {
	        try {
	          ...
	          Result theResult = reader.decode(bitmap, Constants.HINTS);
	          if (theResult != null) {
	            results.add(theResult);
	          }
	        } catch (ReaderException re) {
	      ...
	      if (results.isEmpty()) {
	        try {
	          ...
	          Result theResult = reader.decode(hybridBitmap, Constants.HINTS);
	          if (theResult != null) {
	            results.add(theResult);
	          }
	        } catch (ReaderException re) {
